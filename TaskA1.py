...
class AStarPlanner:
    ...
    def planning(self, sx, sy, st1x, st1y, st2x, st2y, gx, gy):
        """
        A star path search
        input:
            s_x: start x position [m]
            s_y: start y position [m]
            st1_x: stop1 x position [m]
            st1_y: stop1 y position [m]
            st2_x: stop2 x position [m]
            st2_y: stop2 y position [m]
        ...
        stop1_node = self.Node(self.calc_xy_index(st1x, self.min_x), # calculate the index based on given position
                               self.calc_xy_index(st1y, self.min_y), 0.0, -1) # set cost zero, set parent index -1
        stop2_node = self.Node(self.calc_xy_index(st2x, self.min_x), # calculate the index based on given position
                               self.calc_xy_index(st2y, self.min_y), 0.0, -1) # set cost zero, set parent index -1
...
def main():
    ...
    if show_animation:  # pragma: no cover
        ...
        plt.plot(st1x, st1y, "brown") # plot the stop1 position 
        plt.plot(st2x, st2y, "black") # plot the stop2 position 
        plt.plot(gx, gy, "bx") # plot the end position
    ...
    rx, ry = a_star.planning(sx, sy, st1x, st1y, st2x, st2y, gx, gy)
